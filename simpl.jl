...
# switching = ModelingToolkit.SymbolicDiscreteCallback(
#     [1.8] => [
#     direction ~ -1.0
# ];
# discrete_parameters = direction, iv = t)

@mtkcompile sys = System(eqs, t, vars, pars; discrete_events = switching)

prob = ODEProblem(sys, [], (0.0, 3.6); fully_determined = true, guesses = initvals)
sol = solve(prob, Rosenbrock23(), force_dtmin = true)

# display(plot(sol[t], sol[T-273.15], xlabel="sec", ylabel="degC", label="top.T"))
# display(plot(sol[t], sol[p], xlabel="sec", ylabel="Pa", label="top.p"))
# display(plot(sol[t], sol[m], xlabel="sec", ylabel="kg", label="top.m"))
# display(plot(sol[t], sol[m/vol], xlabel="sec", ylabel="kg/m^3", label="top.rho"))
# display(plot(sol[t], sol[Twall-273.15], xlabel="sec", ylabel="degC", label="Twall"))
# display(plot(sol[t], sol[-Q], xlabel="sec", ylabel="W", label="-top.Q", title="Rate of heat transfer from top gas to wall"))